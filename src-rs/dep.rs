...
impl<'a> DepBuilder<'a> {
    ...
    fn pick_rule(&mut self, output: Symbol, n: &Rc<RefCell<DepNode>>) -> Option<PickedRuleInfo> {
        ...
        if let Some(rule_merger) = &rule_merger {
            if rule_merger.borrow_mut().primary_rule.is_some() {
                let mut vars = vars;
                for (sym, _) in &rule_merger.borrow_mut().implicit_outputs {
    ...
    fn build_plan(
        ...
        if let Some(merger) = &picked_rule_info.merger {
            if merger.borrow_mut().parent.is_some() {
                output = merger.borrow_mut().parent_sym.unwrap();
                self.done.insert(output, n.clone());
